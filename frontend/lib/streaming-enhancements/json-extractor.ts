...
export function extractStreamingJSON(content: string): StreamUpdate {
  ...
  if (content.includes('recalculate') || content.includes('too high') ||
      content.includes('over budget') || content.includes('adjust')) {
    statusMessage = 'AI is adjusting the pricing to match your budget...';
  } else if (content.includes('thinking') || content.includes('let me')) {
    statusMessage = 'AI is analyzing requirements and calculating pricing...';
  ...
  return {
    ...
    latestJSON: validJSONBlocks.length > 0 ? validJSONBlocks[validJSONBlocks.length - 1] : undefined,
    totalInvestment: validJSONBlocks.length > 0 ? validJSONBlocks[validJSONBlocks.length - 1].grand_total : undefined,
    statusMessage
  };
}
