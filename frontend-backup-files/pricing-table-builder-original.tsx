...
// Define mandatory roles with their correct full names
const MANDATORY_ROLES = [
    { name: "Tech - Head Of - Senior Project Management", rate: 190 },
    { name: "Tech - Delivery - Project Coordination", rate: 140 },
    { name: "Account Management - (Account Manager)", rate: 150 },
];
...
function SortableRow({
    ...
    // Function to enforce mandatory roles and their ordering
    const enforceMandatoryRoleOrder = (rows: PricingRow[]): PricingRow[] => {
        // 1. Separate headers and non-header rows
        const headers = rows.filter((row) => row.isHeader);
        let nonHeaders = rows.filter((row) => !row.isHeader);

        // 2. Check for mandatory roles and add them if missing
        const roleNames = nonHeaders.map((row) => row.role);

        // Check if Tech - Head Of role is present
        if (!roleNames.includes(MANDATORY_ROLES[0].name)) {
            nonHeaders.unshift({
                id: `mandatory-${Date.now()}-1`,
                role: MANDATORY_ROLES[0].name,
                description: "Project oversight and delivery management",
                hours: 0,
                rate: MANDATORY_ROLES[0].rate,
            });
        }

        // Check if Project Coordination role is present
        if (!roleNames.includes(MANDATORY_ROLES[1].name)) {
            nonHeaders.splice(1, 0, {
                // Insert at position 1 (after Tech Head)
                id: `mandatory-${Date.now()}-2`,
                role: MANDATORY_ROLES[1].name,
                description: "Project coordination and task management",
                hours: 0,
                rate: MANDATORY_ROLES[1].rate,
            });
        }

        // Check if Account Management role is present
        if (!roleNames.includes(MANDATORY_ROLES[2].name)) {
            nonHeaders.push({
                id: `mandatory-${Date.now()}-3`,
                role: MANDATORY_ROLES[2].name,
                description:
                    "Client relationship management and account strategy",
                hours: 0,
                rate: MANDATORY_ROLES[2].rate,
            });
        }

        // 3. Enforce ordering - Tech Head at top, Account Management at bottom
        const reorderedNonHeaders: PricingRow[] = [];

        // First add Tech Head role if it exists
        const techHeadRole = nonHeaders.find(
            (r) => r.role === MANDATORY_ROLES[0].name,
        );
        if (techHeadRole) {
            reorderedNonHeaders.push(techHeadRole);
            nonHeaders = nonHeaders.filter((r) => r.id !== techHeadRole.id);
        }

        // Add Project Coordination role if it exists (after Tech Head)
        const projectCoordRole = nonHeaders.find(
            (r) => r.role === MANDATORY_ROLES[1].name,
        );
        if (projectCoordRole) {
            reorderedNonHeaders.push(projectCoordRole);
            nonHeaders = nonHeaders.filter((r) => r.id !== projectCoordRole.id);
        }

        // Add all other non-mandatory roles
        nonHeaders.forEach((row) => {
            if (!MANDATORY_ROLES.some((mr) => mr.name === row.role)) {
                reorderedNonHeaders.push(row);
            }
        });
        ...
        return (
            ...
            >
                ...
                <div className="col-span-12 md:col-span-1 flex items-center justify-center">
                    {index === 0 && (
                        <Label className="text-xs mb-1 block w-full">
                            Actions
                        </Label>
                    )}
                    <Button
    ...
    const handleDragEnd = (event: DragEndEvent) => {
        ...
        if (over && active.id !== over.id) {
            setRows((items) => {
                ...
                const oldIndex = nonHeaderRows.findIndex(
                    (item) => item.id === active.id,
                );
                const newIndex = nonHeaderRows.findIndex(
                    (item) => item.id === over.id,
                );

                if (oldIndex !== -1 && newIndex !== -1) {
                    const reorderedNonHeaders = arrayMove(
                        nonHeaderRows,
                        oldIndex,
                        newIndex,
                    );
                    ...
                    // Apply mandatory role ordering after drag and drop
                    return enforceMandatoryRoleOrder(result);
                }
    ...
    const calculateSubtotal = () => {
        return rows.reduce((acc, row) => {
            if (!row.isHeader && row.hours > 0) {
                return acc + row.hours * row.rate;
            }
            return acc;
        }, 0);
    };
    ...
    const calculateTotal = () => {
        ...
        // Apply commercial rounding to nearest $100
    ...
    const generateMarkdown = () => {
        ...
        rows.forEach((row) => {
            if (row.isHeader) {
                ...
                markdown += `| ${row.role} | ${row.description} | ${row.hours} | $${row.rate} | $${cost.toFixed(2)} +GST |\n`;
            }
        });
        ...
        markdown += `| **Total Project Value** | **$${calculateTotal().toFixed(2)} +GST** |\n`;
        ...
        <div className="w-full max-w-6xl mx-auto p-6 bg-background border border-border rounded-lg shadow-lg">
            <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex gap-2">
                    <Button
                        onClick={addScopeHeader}
                        size="sm"
                        variant="outline"
                        className="border-[#2C823D] text-[#2C823D] hover:bg-[#2C823D]/10"
                    >
                    ...
                    <Button
                        onClick={addRow}
                        size="sm"
                        className="bg-[#2C823D] hover:bg-[#25703A]"
                    >
                        <Plus className="h-4 w-4 mr-1" />
                        Add Role
                    </Button>
                ...
                <div className="space-y-3 mb-6 max-h-96 overflow-y-auto">
                    ...
                    >
                        {rows.map((row, index) => {
                            ...
                            const nonHeaderRows = rows.filter(
                                (r) => !r.isHeader,
                            );
                            const actualIndex = nonHeaderRows.findIndex(
                                (r) => r.id === row.id,
                            );
